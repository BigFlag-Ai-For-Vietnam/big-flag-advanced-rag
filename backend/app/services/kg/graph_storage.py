...
Extractor -> Validator -> Resolver -> Graph Writer, KHÔNG lọc hậu kỳ (post-hoc).
...
có tài liệu — gắn chặt với đúng version pin trong requirements-kg.txt.
...
-> lệch dữ liệu giữa graph và vector-DB nội bộ của LightRAG. build_service.py phải quan
...
from app.services.kg.ontology.loader import ALLOWED_TRIPLES, canonical_entity_type, classify_relation_keyword

logger = logging.getLogger("kg.graph_storage")

_STORAGE_NAME = "OntologyValidatingGraphStorage"
_MODULE_PATH = "app.services.kg.graph_storage"  # path TUYỆT ĐỐI (không có "." đầu)
...
# Thu thập lại mọi lần drop để build_service.py in báo cáo sau mỗi document — reset qua
# reset_drop_log() trước mỗi ainsert().
DROP_LOG: list[dict] = []
