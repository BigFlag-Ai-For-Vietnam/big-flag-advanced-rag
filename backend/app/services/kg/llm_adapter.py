...
`embedding_func` hàng chục/hàng trăm lần khi build graph — nên dùng thẳng helper
`openai_complete_if_cache` / `openai_embed` có sẵn của LightRAG (đọc base_url/api_key/model
từ `app.config.settings`, cùng nguồn config với backend thật) thay vì đi qua
`app/services/llm_client.py`.

Đây là ngoại lệ CÓ CHỦ ĐÍCH cho riêng build graph: `llm_client.py` là choke point cho mọi
lời gọi LLM khác của app thật (CLAUDE.md "LLM boundary"), nhưng LightRAG cần tự lái
concurrency/retry của chính nó — tái dùng model ID + api_key + base_url từ `settings` để
không lệch config với phần còn lại của repo, không tự dựng `openai.OpenAI()` thô.
"""
...
async def llm_model_func(
    ...
    **kwargs,
    ...
    # — nếu không tắt, build 1 document nhiều chunk có thể mất >15 phút thay vì ~1-2 phút.
