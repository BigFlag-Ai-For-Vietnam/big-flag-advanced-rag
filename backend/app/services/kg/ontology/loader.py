"""Đọc entities.yaml + relations.yaml (ontology tuân thủ ngân hàng, sinh từ catalog +
GROUND_TRUTH lúc PoC — xem `backend/poc/kg_ontology/build_ontology_yaml.py` để biết cách
sinh lại nếu cần mở rộng domain khác), expose interface dùng chung cho:
- `graph_storage.py` (Validator + Resolver, chặn trước khi ghi Neo4j)
- `llm_adapter.py` (entity_types_guidance cho LightRAG extractor)
"""
...
def _load_yaml(path: str) -> dict:
    if not os.path.exists(path):
        raise FileNotFoundError(f"{path} chưa tồn tại — thiếu file ontology.")
...
def classify_relation_keyword(keywords: str) -> str | None:
    ...
    Heuristic substring-match — KHÔNG phải NLP chuẩn. Trả None nếu không khớp type nào ->
    caller (graph_storage) coi cạnh đó là ngoài ontology."""
