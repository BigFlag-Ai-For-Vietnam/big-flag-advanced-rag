...
```bash
python -m eval dataset generate --all --size 20 --personas eval/personas.json --prompts-dir eval/prompts/vi --dataset my-dataset
```
| Flag | Ý nghĩa |
|---|---|
...
| `--dataset <name>` (**bắt buộc**) | Tên MLflow Evaluation Dataset để upload thẳng mẫu retained (FR-16, đường chính rev 3 — xem dưới). |
...
`MultiHopAbstractQuerySynthesizer` / `MultiHopSpecificQuerySynthesizer` (FR-3). Nếu KG (tự
dựng hoặc `--kg-file`) không tạo được cluster nào cho 1 synthesizer multi-hop, `generate`
**không** raise lỗi giữa chừng — trọng số của synthesizer đó được dồn về trivial (single-hop),
in cảnh báo ra console và ghi `multihop_backfill_report.json` làm run artifact (FR-3 r2,
`eval/distribution.py::apply_backfill`). Mỗi run log lên MLflow: `kg.json`,
`language_gate_report.json`, `personas.json`, thư mục `prompts/` đã dùng, `silver.jsonl`, và
`dataset_upload.jsonl` (snapshot đúng những gì đã merge vào `--dataset`) — đầy đủ để trace
lại dataset từ đâu ra (NFR-4). Cổng ngôn ngữ loại mẫu không phải tiếng Việt trước khi ghi
silver; loại > 20% → thoát lỗi + tag `quality_gate=failed` (FR-5, NFR-5) — mẫu retained vẫn
được upload lên `--dataset` trước khi kiểm tra ngưỡng này (retained luôn hợp lệ tiếng Việt,
`failed_quality` chỉ cảnh báo tỉ lệ loại quá cao của lần generate này).

Mẫu retained (post-gate) được upload thẳng vào MLflow Evaluation Dataset tên `--dataset` qua
...
`generate` với cùng `--dataset` merge chứ không nhân đôi record (T06 spike, cùng cơ chế
`promote` dùng — xem cảnh báo record-identity ở cuối mục này).

### `dataset adapt-prompts` — adapt prompt sang tiếng Việt (chạy 1 lần)
```bash
python -m eval dataset adapt-prompts --prompts-dir eval/prompts/vi
```
...
lưu xuống `--prompts-dir` để `dataset generate` tái sử dụng. **Lưu ý**: chất lượng adapt phụ
thuộc LLM — có thể lệch ngôn ngữ (caveat đã biết của ragas); cổng ngôn ngữ trong `generate`
là lưới an toàn cho việc này, không phải cơ chế chính để đảm bảo chất lượng.
...
1. `dataset adapt-prompts` (chạy 1 lần, hoặc lại khi cần) → `eval/prompts/vi/`.
2. `dataset generate --all --personas eval/personas.json --prompts-dir eval/prompts/vi` → sinh
...
4. **Đường chính** (nếu Review Queue khả dụng): `dataset review-queue --dataset <name>`, SME
...
5. `dataset promote --dataset <name>` → gom trace đã `approve=pass`, ưu tiên giá trị HUMAN,
   merge vào `golden-<name>` + JSONL snapshot. Chạy lại an toàn — không nhân đôi record (xem
   cảnh báo record-identity ở §3).
6. `judge --dataset golden-<name>` → xem breakdown metric theo synthesizer/persona, báo cáo
...
- **`inputs` CHỈ chứa đúng `{question, persona_name}`** — không thêm field nào khác (kể cả
  ...
  `inputs` (record identity, không phân biệt thứ tự key — xác nhận sống trên MLflow 3.14, T06
  spike): bất kỳ field thừa nào khiến 2 mẫu ngữ nghĩa giống nhau bị coi là 2 record khác nhau
...
- **CLI chạy trên host** (`dataset generate`/`dataset adapt-prompts`/`dataset review-queue`/
  `dataset promote`/`judge`): mặc định `http://localhost:5000` hoạt động thẳng — CLI với tới
...
## 7. Ghi nhận quyết định các spike
...
- **T06 — `merge_records` semantics**: xác nhận sống trên MLflow 3.14 — `merge_records` là
  **upsert theo nội dung dict `inputs`** (không phân biệt thứ tự key), **không phải append**;
  ...
  `inputs` chỉ được chứa đúng `{"question", "persona_name"}` (xem cảnh báo ở §3). Chi tiết:
