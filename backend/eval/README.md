...
```json
{
  ...
  "source": {"source_type": "CODE", "source_data": {"generator": "llm", "model": "GLM-5.2"}},
  "tags": {"source": "llm", "generation_model": "GLM-5.2"}
}
```
...
- **`source` được set tường minh, không để MLflow tự suy luận**: mọi record của `generate`
  đều có `expectations` khác rỗng, nên nếu bỏ trống `source`, `_infer_source_types` của
  MLflow (`mlflow/entities/evaluation_dataset.py`) sẽ tự gán `source_type=HUMAN` — SAI, vì
  record này do LLM sinh, không phải SME. `dataset_upload.py::build_records` set
  `source_type="CODE"` (giá trị gần nhất trong enum MLflow — không có `LLM`) kèm
  `source_data={"generator": "llm", "model": <model_id>}`; `model_id` = `judge_model_id()`
  (model đã sinh testset qua `TestsetGenerator`, không phải embedding model).
- **`tags.source = "llm"` + `tags.generation_model = <model_id>`**: bản sao đọc-nhanh của
  cùng thông tin (MLflow UI hiển thị `tags` như cột riêng trên bảng record) — không thay thế
  `source`, đi kèm nó.
