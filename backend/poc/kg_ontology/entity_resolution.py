"""Entity Resolver — chạy SAU khi graph đã build (post-processing pass trên Neo4j), gộp
node trùng nhưng bị extract dưới nhiều tên khác nhau. 2 tier khác hẳn nhau, không dùng
chung 1 cơ chế:
...
- KhaiNiem / GiaTriQuyDinh: không có khoá tự nhiên — gộp bằng candidate generation rẻ
  (cosine similarity trên embedding đã có sẵn từ chính pipeline build KG) rồi CHỈ gọi LLM
  xác nhận trên các cặp candidate (không so N² cặp).

Chạy độc lập với `run_kg_build.py` — thao tác trực tiếp trên Neo4j đã có sẵn, không cần
build lại graph.
"""
...
from poc.kg_ontology.llm_adapter import _embed, llm_model_func  # noqa: E402
...
async def main() -> None:
    driver = _driver()
    try:
        print("=== Tier 1: VanBan (regex số hiệu) ===")
        r1 = resolve_vanban(driver)
        print(f"  {r1['so_hieu_groups']} nhóm số hiệu, {len(r1['merges'])} node bị gộp:")
        for so_hieu, dropped, keep in r1["merges"]:
            print(f"    [{so_hieu}] '{dropped}' -> '{keep}'")

        print("\n=== Tier 2: KhaiNiem (embedding candidate + LLM confirm) ===")
        r2 = await resolve_fuzzy_concepts(driver, "KhaiNiem")
        print(f"  {r2['nodes']} node, {r2['candidates']} candidate pair, {len(r2['merges'])} bị gộp:")
        for dropped, keep in r2["merges"]:
            print(f"    '{dropped}' -> '{keep}'")

        print("\n=== Tier 2: GiaTriQuyDinh (embedding candidate + LLM confirm) ===")
        r3 = await resolve_fuzzy_concepts(driver, "GiaTriQuyDinh")
        print(f"  {r3['nodes']} node, {r3['candidates']} candidate pair, {len(r3['merges'])} bị gộp:")
